...
2. **Pull Requests**: Revisar y aprobar contribuciones que cumplan con los estándares PSR-12 definidos en `CONTRIBUTING.md`.
3. **Lanzamientos**: Gestionar el versionado SemVer y la publicación de imágenes en GitHub Packages.
4. **Seguridad**: Atender de forma prioritaria cualquier vulnerabilidad reportada según la `SECURITY.md`.
5. **Monitoreo de Salud**: Verificar periódicamente que los endpoints `/health` y `/ready` de todas las apps respondan correctamente. Usar `make hub-doctor` para diagnóstico completo del entorno.
